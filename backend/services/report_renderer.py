...
<div class="run">
  ...
  <table><thead><tr><th>#</th><th>Tactic</th><th>Techniques</th><th>Status</th></tr></thead>
    ...
    return f'<section><h2 class="sec">3. Methodology (MITRE ATT&CK)</h2>{"".join(blocks)}</section>'


def _findings(findings: List[Dict[str, Any]]) -> str:
    if not findings:
        return ('<section><h2 class="sec">4. Findings</h2>'
...
<div class="finding" style="border-left-color:{SEV_COLORS.get(sev, SEV_COLORS['info'])[0]}">
  <div class="head">
    ...
    {_sev_badge(sev)} <span class="muted">CVSS {cvss}</span> {_verified_badge(f)}
  </div>
  ...
  {vv_html}{cmds_html}{ev_html}
    ...
    return f'<section><h2 class="sec">4. Findings</h2>{"".join(cards)}</section>'
...
def _appendix(runs: List[Dict[str, Any]], verdicts_by_run: Dict[str, List[Dict[str, Any]]]) -> str:
    ...
    if not blocks:
        return ""
    return f'<section><h2 class="sec">5. Evidence Appendix</h2>{"".join(blocks)}</section>'
